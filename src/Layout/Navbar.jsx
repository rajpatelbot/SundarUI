...
export default function Navbar() {
  ...
  return (
    <header className="bg-white padding-px">
      ...
      <nav className="mx-auto flex maxWidth items-center justify-between py-6" aria-label="Global">
              ...
              <Popover.Panel className="absolute -left-8 top-full z-10 mt-3 w-screen max-w-md overflow-hidden rounded-3xl bg-white shadow-lg ring-1 ring-gray-900/5">
                <div className="p-4">
                  {submenuActions.map((item) => (
                    <div key={item.name} className="group relative flex items-center gap-x-6 rounded-lg p-4 leading-6">
        ...
        <div className="hidden lg:flex lg:flex-1 lg:justify-end">
          <a href="https://github.com/rajpatel17-bot/SundarUI" target={'_blank'} className="text-5xl">
            <BsGithub />
          </a>
        </div>
      </nav>
          ...
          <div className="mt-6 flow-root">
            <div className="-my-6 divide-y divide-gray-500/10">
              <div className="space-y-2 py-6">
                <Link href="/setupguide" className="-mx-3 block rounded-lg py-2 px-3 leading-7">
                  Setup Guide
                </Link>
                  ...
                  {({ open }) => (
                    <>
                      <Disclosure.Button className="flex w-full items-center justify-between rounded-lg py-2 pl-3 pr-3.5 leading-7">
                        Components
                        <AiFillCaretDown
                          className={classNames(open ? 'rotate-180' : '', 'h-10 w-10 flex-none')}
                          aria-hidden="true"
                        />
                      </Disclosure.Button>
